...
| # | Item | Blocker |
|---|---|---|
| **A8** | `/api/esc-rpc` TLS pinning (replace `rejectUnauthorized:false`) | **Decision locked (2026-04-23)**: Option 1 (hostname + public-CA verification). Live probing of `38.242.211.112:443` confirmed the box already serves a valid Let's Encrypt **`*.ela.city`** wildcard cert (issued 2026-02-20, expires 2026-05-21, auto-renewing) — the in-code comment ("self-signed cert") is wrong. **Chosen hostname**: `elastossmartchain.ela.city` (Sash's pick — `esc.ela.city` is already in use). **Blocker**: Sash is travelling and the DNS provider requires SMS 2FA on a number he can't reach right now. Parked until Sash is back at his usual SIM. **Action when unblocked**: add `A elastossmartchain.ela.city → 38.242.211.112` (TTL 30 min); the agent then waits for propagation, switches the proxy + 4 other call sites (`chipotle-client.ts`, `ConnectivityService.ts`, two more) to the hostname, removes 5x `rejectUnauthorized:false`, and pushes one atomic commit. Confirmed `https://38.242.211.112/rpc/esc` returns valid `eth_blockNumber` today, so the endpoint itself is healthy. |
