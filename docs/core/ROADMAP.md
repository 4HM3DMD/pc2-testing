...
- [x] Creator Dashboard dApp — upload any file, set price/royalties, encrypt via Lit Protocol, upload to IPFS *(implemented Mar 13 — pc2-node/data/test-apps/elacity-creator/)*
  ...
  - [ ] **WASM crypto hardening** — P-256 ECDH unwrap, AES-GCM decrypt, fMP4 strip+decrypt combined, PDF text extraction. See `.cursor/tasks/WASM-CRYPTO-HARDENING/`. Branch: `feature/wasm-crypto-hardening`
