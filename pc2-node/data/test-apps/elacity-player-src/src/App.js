...
export const App = () => {
    const [provider, setProvider] = React.useState(window.ethereum);
    ...
    React.useEffect(() => {
        ...
        let cancelled = false;
        const puterArgs = window.puter?.args;
    ...
    if (!provider || !address) {
        return (<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', color: '#94a3b8', gap: 16, padding: 32, textAlign: 'center' }}>
    ...
    return (<PlayerProvider provider={provider} address={address}>
      <PlayerView />
    </PlayerProvider>);
};
