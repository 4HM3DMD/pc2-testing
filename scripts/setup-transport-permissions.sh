...
# wg-quick and awg-quick (when awg-quick is bundled), AND that they use the
# v1.2.7.10 SETENV: form (so 'sudo -E' callers carrying
# WG_QUICK_USERSPACE_IMPLEMENTATION succeed). The file has 0440 perms so
# non-root can't read it — use sudo for the read check.
needs_install="no"
...
if sudo -n test -f "$SUDOERS_FILE" 2>/dev/null; then
    ...
    if echo "$sudoers_content" | grep -q -F "$WG_QUICK"; then
        # Check for SETENV flag — required as of v1.2.7.10 so wg-quick can
        # locate bundled wireguard-go via env var under sudo. Pre-v1.2.7.10
        # entries lack this flag → fall through to reinstall.
        if ! echo "$sudoers_content" | grep -q -i 'SETENV'; then
            echo "↻ Sudoers entry exists but missing SETENV flag (pre-v1.2.7.10) — upgrading in-place."
            needs_install="yes"
        elif [[ -x "$AWG_QUICK" ]]; then
            if echo "$sudoers_content" | grep -q -F "$AWG_QUICK"; then
                echo "✓ Transport sudoers already complete (wg-quick + awg-quick, SETENV) — skipping."
                exit 0
            else
            ...
            echo "✓ Sudoers entry already configured for wg-quick with SETENV (awg-quick not bundled) — skipping."
            exit 0
        fi
    else
    ...
    # Note: this won't catch the pre-v1.2.7.10 → SETENV upgrade case here
    # (because plain `sudo wg-quick --version` works on both pre/post .10
    # entries), but the runtime checkWireGuardPermissions() in
    # setupPermissions.ts catches it on the next pc2-node start and
    # triggers the osascript prompt to upgrade.
    if sudo -n "$WG_QUICK" --version >/dev/null 2>&1; then
        echo "✓ Passwordless sudo verified for wg-quick — skipping (runtime will upgrade SETENV if missing)."
        exit 0
    fi
    needs_install="yes"
else
...
# re-prompt the user.
#
# v1.2.7.10: rules use NOPASSWD:SETENV: instead of plain NOPASSWD:. The
# SETENV flag is required because wg-quick on macOS needs to find its
# bundled wireguard-go companion, which sits at
# ~/.pc2/pc2-node/bin/<platform>-<arch>/ — outside sudo's secure_path of
# /usr/bin:/bin:/usr/sbin:/sbin. We tell wg-quick where it is via env var
# (WG_QUICK_USERSPACE_IMPLEMENTATION=...), and SETENV permits that under
# `sudo -E`. Same applies to awg-quick → amneziawg-go.
USER_NAME="$(whoami)"
...

{
    ...
    echo "#"
    echo "# v1.2.7.10: rules use NOPASSWD:SETENV: so 'sudo -E' callers can pass"
    echo "# WG_QUICK_USERSPACE_IMPLEMENTATION=<bundled-wireguard-go-path>. Without"
    echo "# SETENV, sudo strips the var and wg-quick can't locate our bundled"
    echo "# wireguard-go / amneziawg-go (sudo's secure_path doesn't include"
    echo "# ~/.pc2/pc2-node/bin/<platform>-<arch>/)."
    echo "#"
    ...
    echo "$USER_NAME ALL=(root) NOPASSWD:SETENV: $WG_QUICK up *"
    echo "$USER_NAME ALL=(root) NOPASSWD:SETENV: $WG_QUICK down *"
    if [[ -x "$AWG_QUICK" ]]; then
        echo "$USER_NAME ALL=(root) NOPASSWD:SETENV: $AWG_QUICK up *"
        echo "$USER_NAME ALL=(root) NOPASSWD:SETENV: $AWG_QUICK down *"
    fi
} > "$TMPFILE"
